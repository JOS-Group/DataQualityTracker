...
#   • Persona-driven onboarding & dynamic page visibility
#   • Settings page to update persona
...
#   • AI Copilot & Automation templates
# ============================================================
...
if st.session_state.persona is None:
    ...
    st.caption("A few questions personalize what you see first. Your selections control which features and pages are visible.")
...
with st.sidebar.expander("My profile"):
    ...
    st.caption("Change your persona in the **Settings** page to view different tools.")
...
# --- DYNAMIC PAGE VISIBILITY based on Persona ---
# Core pages everyone gets
pages = ["Home", "My Briefing", "Compare"]

focus = p.get("focus", [])
role = p.get("role", "")

if "System reconciliation" in focus or role in ["Data / Analytics", "IT / Engineering"]:
    pages.extend(["System Explorer", "Connections"])

if "Data quality" in focus or role in ["Data / Analytics", "Quality / Compliance", "Operations"]:
    pages.append("Data Quality Lab")

if "Issues & remediation" in focus or role in ["Operations", "Quality / Compliance", "Program / Policy"]:
    pages.append("Issue Studio")

if "Reporting impact" in focus or role in ["Executive / Leadership", "Program / Policy", "Data / Analytics"]:
    pages.append("Report Impact")

if "AI insights" in focus or role in ["Executive / Leadership", "Data / Analytics", "Research"]:
    pages.append("AI Copilot")

if "Automation" in focus or role in ["IT / Engineering", "Data / Analytics"]:
    pages.append("Automations")

# Append universally available data/settings pages at the end
pages.extend(["My Data", "Settings"])
...
if page == "Home":
    ...
    if "System Explorer" in pages:
        x.button("Use System Explorer", key="home_explore")
    else:
        x.caption("Enable 'System reconciliation' in Settings to view this feature.")
        
    y.subheader("Act")
    y.write("Turn mismatches into issues, owners, priorities, and recommendations.")
    if "Issue Studio" in pages:
        y.button("Open Issue Studio", key="home_issue")
    else:
        y.caption("Enable 'Issues & remediation' in Settings to view this feature.")

    z.subheader("Ask")
    z.write("Use natural language to investigate the current data.")
    if "AI Copilot" in pages:
        z.button("Open AI Copilot", key="home_ai")
    else:
        z.caption("Enable 'AI insights' in Settings to view this feature.")
    ...
    st.download_button(label="Download Filtered View", data=csv, file_name="medi_intelligence_filtered.csv", mime="text/csv")
...
# ------------------------- ISSUE STUDIO -------------------------
    ...
    if st.session_state.issues:
        ...
        st.download_button("Export Issues CSV", data=csv_issues, file_name="medi_issues_export.csv", mime="text/csv")
...
# ------------------------- MY DATA -------------------------
    ...
    st.markdown('<div class="hero"><h1>My Data</h1><p>Inspect the underlying raw dataset, view schema details, and export the full file.</p></div>',unsafe_allow_html=True)
    
    a,b,c = st.columns(3)
    a.metric("Active Dataset Source", st.session_state.source)
    b.metric("Total Rows", f"{len(st.session_state.df):,}")
    c.metric("Total Columns", f"{len(st.session_state.df.columns):,}")
    
    st.markdown("### Full Dataset Preview")
    st.dataframe(st.session_state.df.head(1000), use_container_width=True)
    
    col1, col2 = st.columns([1, 4])
    csv_raw = st.session_state.df.to_csv(index=False).encode('utf-8')
    col1.download_button(label="Download Full Dataset (CSV)", data=csv_raw, file_name="medi_intelligence_fulldata.csv", mime="text/csv", type="primary")

# ------------------------- SETTINGS -------------------------

elif page == "Settings":
    st.markdown('<div class="hero"><h1>Workspace Settings</h1><p>Update your persona details to instantly change which features and pages are visible in the sidebar menu.</p></div>',unsafe_allow_html=True)
    
    # Provide safe indexes to avoid ValueError if session state value doesn't exactly match the options
    prog_opts = ["Medicare","Medicaid","Both","Other / General"]
    role_opts = ["Executive / Leadership","Data / Analytics","Program / Policy","Operations","Quality / Compliance","IT / Engineering","Research","Other"]
    aud_opts = ["Myself","Leadership","Analysts","Program teams","Technical teams","External stakeholders"]
    depth_opts = ["Executive","Balanced","Technical"]
    
    prog_idx = prog_opts.index(p["program"]) if p["program"] in prog_opts else 0
    role_idx = role_opts.index(p["role"]) if p["role"] in role_opts else 0
    aud_idx = aud_opts.index(p["audience"]) if p["audience"] in aud_opts else 0
    depth_idx = depth_opts.index(p["depth"]) if p["depth"] in depth_opts else 1

    with st.form("settings_form"):
        st.subheader("Your Persona Profile")
        a,b = st.columns(2)
        with a:
            new_program = st.radio("Program of Interest", prog_opts, index=prog_idx, horizontal=True)
            new_role = st.selectbox("Primary role", role_opts, index=role_idx)
            new_focus = st.multiselect("Focus Areas (Controls what pages appear)",[
                "Data quality","Claims","System reconciliation","State comparisons",
                "Issues & remediation","Business rules","Reporting impact","Automation","AI insights"
            ], default=p["focus"])
        with b:
            new_geography = st.multiselect("Geography", ["National","State","Multi-state","Specific states"], default=p["geography"])
            new_audience = st.selectbox("Primary Audience", aud_opts, index=aud_idx)
            new_depth = st.select_slider("Detail Level", depth_opts, value=depth_opts[depth_idx])
            new_first_question = st.text_input("Primary Question", value=p["first_question"])
            
        if st.form_submit_button("Update Workspace Persona", type="primary"):
            st.session_state.persona = {
                "program": new_program, "role": new_role, "focus": new_focus, 
                "geography": new_geography, "audience": new_audience, 
                "depth": new_depth, "first_question": new_first_question
            }
            st.success("Persona updated! The sidebar menu has been updated to reflect your new capabilities.")
            st.rerun()

    st.markdown("---")
    st.markdown("### How Persona Routing Works")
    st.info("""
    - **System Explorer & Connections:** Visible for *IT / Data roles* or if *System reconciliation* is focused.
    - **Data Quality Lab:** Visible for *Data / Quality / Operations roles* or if *Data quality* is focused.
    - **Issue Studio:** Visible for *Quality / Operations roles* or if *Issues & remediation* is focused.
    - **Report Impact:** Visible for *Executive / Program roles* or if *Reporting impact* is focused.
    - **AI Copilot:** Visible for *Executive / Data roles* or if *AI insights* is focused.
    - **Automations:** Visible for *IT / Data roles* or if *Automation* is focused.
    """)