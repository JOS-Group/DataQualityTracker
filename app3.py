...
- Distinct weighted-volume chart and stronger chart validation guards
...
def standardize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    aliases = {
        ...
        "MatchedClaims": ["MatchedClaims", "Matched Claims", "Matched Events", "Matched", "Match Count"],
        "NotMatchedClaims": ["NotMatchedClaims", "Not Matched Claims", "Not-Matched Claims", "Unmatched", "Mismatch Count"],
        "MatchRate": ["MatchRate", "Match Rate", "Match %", "Match Percentage", "Accuracy"],
    ...
    for col in [
        ...
        "Disposition", "Recommendation", "Comments", "Sub-Classification", "Owner", "Report", "KPI",
    ]:
        if col not in df:
            df[col] = ""
...
def demo_data() -> pd.DataFrame:
    ...
    meta = [
        ...
        ("HH", "Home Health", "FISS"),
        ("HSPC", "Hospice", "FISS"),
        ("DME", "Durable Medical Equipment", "VMS"),
    ]
    reasons = ["Type mapping", "Date logic", "Null treatment", "Code set", "Join key", "Timing lag"]
    rows: List[Dict[str, Any]] = []
    for stream, domain, system in meta:
        for i in range(34):
            base = float(rng.uniform(0.42, 0.995))
            matched = int(rng.integers(2500, 240000))
            total = max(int(matched / max(base, 0.05)), matched)
            report = f"{stream} Quality Report" if i % 4 else "Claims Integrity Dashboard"
            kpi = f"{stream} Match KPI" if i % 5 else "Executive Data Quality KPI"
            rows.append({
                "Stream": stream,
                "NCH Target Table": f"{stream}_CLAIM_TABLE_{(i % 6) + 1}",
                "NCH Target Column": f"{stream}_FIELD_{i + 1:02d}",
                "SS Table": f"{system}_TARGET_TABLE_{(i % 6) + 1}",
                "SS Column": f"{system}_FIELD_{i + 1:02d}",
                "MatchedClaims": matched,
                "NotMatchedClaims": total - matched,
                "MatchRate": matched / total,
                "Classification": "Match" if base >= 0.92 else ("Different" if base >= 0.62 else "Missing"),
                "Sub-Classification": reasons[i % len(reasons)],
                "Disposition": "Good" if base >= 0.92 else "Action needed",
                "Recommendation": "No action" if base >= 0.92 else f"Review {reasons[i % len(reasons)].lower()} and confirm lineage",
                "Comments": f"Demo record for {domain}",
                "Owner": ["Data Quality", "Claims Ops", "Reporting", "Engineering"][i % 4],
                "Report": report,
                "KPI": kpi,
            })
...
def load_local_data() -> Tuple[pd.DataFrame, str]:
    ...
    for root in [here, here / "data"]:
        if root.exists():
            ...
            files.extend(root.glob("*.xls"))
    files = sorted(files, key=lambda p: (0 if any(k in p.name.lower() for k in ["recon", "comparison", "integrity", "dart"]) else 1, p.name))
    for path in files:
        try:
            if path.name == USERS_FILE.name:
                continue
            if path.suffix.lower() == ".csv":
                df = pd.read_csv(path)
                if len(df):
                    return standardize(df), path.name
            else:
                xl = pd.ExcelFile(path)
                for sheet in xl.sheet_names:
                    df = pd.read_excel(path, sheet_name=sheet)
                    if len(df) and len(df.columns) >= 3:
                        return standardize(df), f"{path.name} / {sheet}"
        except Exception:
            pass
    return pd.DataFrame(), ""
...
def ensure_data() -> None:
    if STATE["df"].empty:
        ...
        if df.empty:
            STATE["df"] = demo_data()
            STATE["source"] = "Built-in DART demo data"
        else:
            STATE["df"] = df
            STATE["source"] = name
    df = standardize(STATE["df"])
    seed_lineage_from_df(df)
    seed_workflow_items(df)
    seed_impacts(df)
    seed_governance()
...
def starter_actions() -> List[Dict[str, Any]]:
    return [
        {"Stream": "PROF", "NCH Target Column": "PROF_FIELD_07", "MatchRate": 0.782, "NotMatchedClaims": 18420, "RiskTier": "Elevated", "ImpactScore": 4.8, "Recommendation": "Starter insight: validate mapping logic and confirm downstream KPI usage."},
        {"Stream": "OUT", "NCH Target Column": "OUT_FIELD_13", "MatchRate": 0.694, "NotMatchedClaims": 22610, "RiskTier": "Critical", "ImpactScore": 6.2, "Recommendation": "Starter insight: review join key and date logic before executive reporting."},
        {"Stream": "DME", "NCH Target Column": "DME_FIELD_04", "MatchRate": 0.811, "NotMatchedClaims": 9710, "RiskTier": "Watch", "ImpactScore": 2.7, "Recommendation": "Starter insight: monitor for repeat variance in the next reconciliation cycle."},
    ]


def starter_streams() -> List[Dict[str, Any]]:
    return [
        {"Stream": "PROF", "Fields": 34, "AverageMatch": 0.88, "WeightedMatch": 0.91, "Matched": 3200000, "NotMatched": 245000, "AverageImpact": 2.9, "Critical": 2},
        {"Stream": "OUT", "Fields": 34, "AverageMatch": 0.82, "WeightedMatch": 0.86, "Matched": 2800000, "NotMatched": 410000, "AverageImpact": 3.7, "Critical": 4},
        {"Stream": "DME", "Fields": 34, "AverageMatch": 0.90, "WeightedMatch": 0.92, "Matched": 1600000, "NotMatched": 130000, "AverageImpact": 2.1, "Critical": 1},
    ]


def summary_payload(fdf: pd.DataFrame) -> Dict[str, Any]:
    if fdf.empty:
        return {"rows": 0, "field_average": "-", "weighted_rate": "-", "actions": "0", "streams": "0", "unmatched": "0", "risk_count": "0", "health_score": "-", "critical": "0", "risk_score": "-", "affected_reports": "0", "top_actions": starter_actions(), "stream_summary": starter_streams()}
    ...
    stream_summary["WeightedMatch"] = stream_summary["Matched"] / (stream_summary["Matched"] + stream_summary["NotMatched"]).replace(0, np.nan)
...
def build_charts(fdf: pd.DataFrame) -> Tuple[Dict[str, str], List[Dict[str, Any]]]:
    ...
    stream["WeightedMatch"] = stream["Matched"] / (stream["Matched"] + stream["NotMatched"]).replace(0, np.nan)
    compare_table = stream.round(4).replace({np.nan: None}).to_dict("records")
    fig = px.bar(stream.sort_values("WeightedMatch"), x="Stream", y="WeightedMatch", text="WeightedMatch", title="Event-Weighted Match Rate by Stream")
    ...
    fig = px.scatter(stream, x="AverageMatch", y="WeightedMatch", size="Fields", color="Stream", hover_data=["Matched", "NotMatched", "AverageImpact"], title="Field Average vs Event-Weighted Match")
    fig.update_xaxes(tickformat=".0%")
    fig.update_yaxes(tickformat=".0%")
...
async function restoreDemo(){await postJson('/api/restore-demo',{});toast('Demo restored');await render();}
...
async def upload(file: UploadFile = File(...)) -> Any:
    ...
    try:
        if lower.endswith(".csv"):
            ...
            return JSONResponse({"error": "Upload a CSV, XLSX, or XLS file."}, status_code=400)
    except Exception as exc:
        return JSONResponse({"error": f"Could not read file: {exc}"}, status_code=400)
    STATE["df"] = standardize(df)
    STATE["source"] = name
    STATE["snapshots"] = []
    STATE["connections"] = []
    STATE["issues"] = []
...
async def restore_demo() -> Dict[str, str]:
    STATE["df"] = demo_data()
    STATE["source"] = "Built-in DART demo data"
    STATE["snapshots"] = []
    STATE["connections"] = []
...
async def copilot(request: Request) -> Dict[str, str]:
    ...
    if OpenAI and api_key:
        try:
            ...
            answer = f"AI service error: {exc}\n\nLocal result: field average is {pct(field_average(fdf))}; event-weighted rate is {pct(weighted_rate(fdf))}."
    else:
        if fdf.empty:
            ...
            answer = f"Current scope has {len(fdf):,} fields. Field average is {pct(field_average(fdf))}; event-weighted match is {pct(weighted_rate(fdf))}. The workspace also has {len(STATE.get('connections', []))} lineage mappings, {len(STATE.get('impacts', []))} impact mappings, and {len(STATE.get('governance', []))} governance controls. Top action candidates:\n{bullets}\n\nAdd GROQ_API_KEY for model-backed Dartboard analysis."
