...
- Build Your Own uses one navbar source only: DIY Home, Dataset Library, Upload Data, Raw Data Editor, and Email Alerts are direct visible tabs; duplicate persistent/in-page BYO nav strips removed
...
APP_VERSION = "8.9.0"
BUILD_FINGERPRINT = "BYO-NAV-SINGLE-8.9.0"
...
<head>
  ...
  <title>DART 8.9.0 - Data Assurance Reconciliation Tracker</title>
  <script>
    (function(){
      try{
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:radial-gradient(circle at 12% -8%,rgba(249,217,118,.18),transparent 32%),radial-gradient(circle at 85% 0%,rgba(212,175,55,.12),transparent 30%),linear-gradient(180deg,#100c05,#171006 45%,#0b0804);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,Segoe UI,Arial,sans-serif}a{color:inherit;text-decoration:none}
    ...
    .byoDirectTab{display:inline-flex!important;align-items:center!important;gap:6px!important;white-space:nowrap!important}
    .byoDirectTabNew{border-color:#d4af37!important;background:linear-gradient(180deg,rgba(249,217,118,.14),rgba(58,44,16,.28))!important;color:#fff1b5!important;font-weight:900!important}
    .byoDirectTabNew:hover,.byoDirectTabNew.active{background:linear-gradient(135deg,#765700,#ad8208,#d4af37)!important;color:#fff!important;border-color:#f0cd65!important;box-shadow:0 8px 22px rgba(212,175,55,.18)!important}
    .byoTopNewBadge{display:inline-flex;align-items:center;border-radius:999px;padding:1px 5px;background:#f9d976;color:#463400;font-size:.52rem;font-weight:950;letter-spacing:.04em}
    :root[data-theme="dark"] .byoDirectTabNew{background:#29220f!important;color:#f6df94!important;border-color:#9d7e29!important}
    :root[data-theme="dark"] .byoDirectTabNew.active{background:linear-gradient(135deg,#6f5200,#967106,#c49312)!important;color:#fff!important}
    @media(max-width:1100px){.byoDirectTab{padding:8px 9px!important;font-size:.82rem!important}.byoTopNewBadge{display:none}}.menu{position:relative}.menuBtn{border:1px solid transparent;background:transparent;color:var(--muted);padding:9px 12px;border-radius:999px;cursor:pointer;font-size:.9rem;transition:background .18s ease,border-color .18s ease,color .18s ease,transform .18s ease}.menuBtn:hover,.menuBtn.active,.menu.open .menuBtn{color:#fff8cf;border-color:#d4af37;background:#3a2c10;transform:translateY(-1px)}.menuPanel{position:absolute;right:0;top:48px;background:rgba(27,20,8,.98);border:1px solid rgba(212,175,55,.42);border-radius:18px;box-shadow:0 28px 78px rgba(0,0,0,.45);min-width:255px;padding:10px;z-index:180;opacity:0;visibility:hidden;transform:translateY(-7px) scale(.985);pointer-events:none;transition:opacity .18s ease,transform .18s ease,visibility .18s ease}.menuPanel:before{content:"";position:absolute;left:0;right:0;top:-18px;height:18px}.menu.open .menuPanel{opacity:1;visibility:visible;transform:translateY(0) scale(1);pointer-events:auto}.menuPanel a{display:flex;align-items:center;gap:10px;padding:11px 12px;border-radius:12px;color:#decf91;cursor:pointer;transition:background .16s ease,color .16s ease,transform .16s ease}.menuPanel a:hover{background:#33250d;color:#fff7d1;transform:translateX(3px)}.wrap{max-width:1600px;margin:auto;padding:24px 22px 55px;animation:pageFade .28s ease both}
    ...
    /* DART 8.9.0: Build Your Own navigation has one source of truth: #navlinks.
       No persistent BYO row and no render-time BYO tab strip are used. */
...
function activeNavGroups(){return navSets[state.workspace]||navSets.medicare;}
...
document.addEventListener('click',e=>{closeAllMenus();if(!e.target.closest('.workspaceSwitch'))closeWorkspaceMenu();if(!e.target.closest('.smartMulti'))closeSmartMultis();});document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeAllMenus();closeWorkspaceMenu();closeFilterModal();closeMetricModal();closeSmartMultis();}});
function setNav(){
  ...
  if(state.workspace==='byo'){
    const direct=[
      ['byo-home','DIY Home'],
      ['byo-library','Dataset Library'],
      ['byo-lab','Upload Data'],
      ['byo-raw','Raw Data Editor'],
      ['byo-email','Email Alerts']
    ];
    const directHtml=direct.map(([id,text])=>`<button type="button" class="menuBtn byoDirectTab ${state.page===id?'active':''} ${(id==='byo-raw'||id==='byo-email')?'byoDirectTabNew':''}" onclick="showPage('${id}')">${text}${(id==='byo-raw'||id==='byo-email')?'<span class="byoTopNewBadge">NEW</span>':''}</button>`).join('');
    const remaining=groups.filter(g=>g[0]!=='Workspace');
    const groupedHtml=remaining.map(g=>`<div class="menu ${pageInGroup(g)?'current':''}"><button class="menuBtn ${pageInGroup(g)?'active':''}" onclick="toggleMenu(event,this.parentElement)" aria-haspopup="true" aria-expanded="false">${g[0]} ▾</button><div class="menuPanel">${g[1].map(([id,text])=>`<a onclick="showPage('${id}');closeAllMenus()">${text}</a>`).join('')}</div></div>`).join('');
    nav.innerHTML=directHtml+groupedHtml;
    return;
  }
...
async function renderMedicaid(meta){
  ...
  const appEl=document.getElementById('app');appEl.classList.remove('page-enter');appEl.innerHTML=html;runInjectedScripts(appEl);decorateMetricHelp(appEl);refreshThemeVisuals(appEl);requestAnimationFrame(()=>appEl.classList.add('page-enter'));
}
...
async function renderByo(meta,info){
  ...
  const appEl=document.getElementById('app');appEl.innerHTML=html;runInjectedScripts(appEl);decorateMetricHelp(appEl);refreshThemeVisuals(appEl);
}
...
async function render(){const meta=await api('/api/meta');state.meta=meta;if(!meta.auth_user){renderAuth();return;}if(!meta.persona){renderOnboarding();return;}setNav();if(state.page==='profile'||state.page==='settings'){renderSharedPage(meta);return;}if(state.workspace==='medicaid'){await renderMedicaid(meta);return;}if(state.workspace==='byo'){const byo=await api('/api/byo');await renderByo(meta,byo);return;}const data=await getData();let emailInfo=null;let rawInfo=null;if(state.page==='emailagent')emailInfo=await api('/api/email-agent');if(state.page==='raweditor')rawInfo=await api(`/api/raw-data?search=${encodeURIComponent(state.rawSearch)}&offset=${state.rawOffset}&limit=${state.rawLimit}`);document.getElementById('footer').innerHTML=`Loaded: ${esc(meta.source)} · ${intFmt(meta.rows)} rows · ${intFmt(meta.columns)} columns · DART ${esc(meta.version)} · ${esc(meta.build_fingerprint||'build unknown')} · FastAPI`;const p=meta.persona||{};let html=filterBar(meta);
if(state.page==='home'){let body=`${section('overview','Overview',`<div class="hero"><h1>DART Command Center</h1><p>A consolidated view of data quality, reconciliation risk, unmatched volume, remediation activity, and downstream business impact${p.program||p.role?` for <b>${esc(p.program||'your program')}</b> with a <b>${esc(p.role||'general')}</b> role lens`:''}. Use this page as your primary operational command center.</p><div class="heroActions">${personaQuickActions(meta)}</div></div>${metrics(data.summary)}<br>${miniDeck(data.summary)}<br>${personaLensPanel(meta)}`)}${section('riskmix','Risk distribution and quality trend',`<div class="grid grid2"><div class="panel"><h3>Risk distribution</h3>${data.charts.risk}</div><div class="panel"><h3>Quality trend</h3>${data.charts.trend}</div></div>`)}${section('volume','Reconciliation volume',`<div class="grid grid2"><div class="panel"><h3>Unmatched volume</h3>${data.charts.volume}</div><div class="panel"><h3>Reconciliation volume by stream</h3>${data.charts.weighted}</div></div>`)}${section('queue','Priority queue',`<div class="panel"><h3>Priority queue</h3>${cards(data.summary.top_actions.slice(0,personaPriorityLimit(meta)))}</div>`)}`;html+=sectionShell([['overview','Overview'],['riskmix','Risk mix'],['volume','Volume'],['queue','Priority queue']],body);}
...
def build_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "html_has_direct_byo_nav": "byoDirectTab" in HTML,
